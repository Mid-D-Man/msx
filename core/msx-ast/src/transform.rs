...
/// Column-major 2D affine matrix — SVG matrix(a,b,c,d,e,f).
/// [ a  c  e ]
/// [ b  d  f ]
/// [ 0  0  1 ]
...
impl Matrix2D {
    pub fn identity() -> Self { Matrix2D { a: 1.0, b: 0.0, c: 0.0, d: 1.0, e: 0.0, f: 0.0 } }
    pub fn translate(tx: f64, ty: f64) -> Self { Matrix2D { a: 1.0, b: 0.0, c: 0.0, d: 1.0, e: tx, f: ty } }
    pub fn scale(sx: f64, sy: f64)     -> Self { Matrix2D { a: sx,  b: 0.0, c: 0.0, d: sy,  e: 0.0, f: 0.0 } }
    ...
    pub fn rotate_deg_around(angle_deg: f64, cx: f64, cy: f64) -> Self {
        Matrix2D::translate(cx, cy)
            .concat(Matrix2D::rotate_deg(angle_deg))
            .concat(Matrix2D::translate(-cx, -cy))
    }

    pub fn skew_x(a: f64) -> Self { Matrix2D { a: 1.0, b: 0.0, c: a.to_radians().tan(), d: 1.0, e: 0.0, f: 0.0 } }
    pub fn skew_y(a: f64) -> Self { Matrix2D { a: 1.0, b: a.to_radians().tan(), c: 0.0, d: 1.0, e: 0.0, f: 0.0 } }

    /// Concatenate: self * other (other applied first).
    pub fn concat(self, o: Matrix2D) -> Self {
        Matrix2D {
            a: self.a * o.a + self.c * o.b,   b: self.b * o.a + self.d * o.b,
            c: self.a * o.c + self.c * o.d,   d: self.b * o.c + self.d * o.d,
    ...
    pub fn is_identity(self) -> bool {
        const E: f64 = 1e-10;
        (self.a - 1.0).abs() < E && self.b.abs() < E && self.c.abs() < E
            && (self.d - 1.0).abs() < E && self.e.abs() < E && self.f.abs() < E
    }

    pub fn to_svg_attr(self) -> String {
        format!("matrix({},{},{},{},{},{})", fmt_f64(self.a), fmt_f64(self.b), fmt_f64(self.c), fmt_f64(self.d), fmt_f64(self.e), fmt_f64(self.f))
    }

    /// 24-byte wire format: [f32 a b c d e f].
    pub fn to_bytes(self) -> [u8; 24] {
        let mut buf = [0u8; 24];
        for (i, v) in [self.a, self.b, self.c, self.d, self.e, self.f].iter().enumerate() {
...
pub enum Transform {
    ...
    SkewX(f64),
    SkewY(f64),
    /// Chain applied right-to-left (last element first).
...
impl Transform {
    ...
    pub fn to_matrix(&self) -> Matrix2D {
        match self {
            ...
            Transform::Rotate { angle, cx, cy }   => match (cx, cy) {
    ...
    pub fn to_svg_attr(&self) -> String {
        match self {
            ...
            Transform::Scale { x, y }             => if (x - y).abs() < 1e-10 { format!("scale({})", fmt_f64(*x)) } else { format!("scale({},{})", fmt_f64(*x), fmt_f64(*y)) },
            Transform::Rotate { angle, cx, cy }   => match (cx, cy) {
            ...
            Transform::Multiple(v) => v.iter().map(|t| t.to_svg_attr()).collect::<Vec<_>>().join(" "),
        }
    }
    ...
    pub fn parse_svg(s: &str) -> Self {
        ...
        let mut parts: Vec<Transform> = s.split(')').filter_map(|chunk| {
            let chunk = chunk.trim();
            if chunk.is_empty() { return None; }
            let paren = chunk.find('(')?;
            let name  = chunk[..paren].trim().to_lowercase();
            let args: Vec<f64> = chunk[paren+1..].trim()
                .split([',', ' '].as_ref())
                .filter(|s| !s.is_empty())
                .filter_map(|s| s.trim().parse().ok())
                .collect();
            match name.as_str() {
                "translate" => Some(Transform::Translate { x: *args.first()?, y: args.get(1).copied().unwrap_or(0.0) }),
                "scale"     => Some(Transform::Scale { x: *args.first()?, y: args.get(1).copied().unwrap_or(*args.first()?) }),
                "rotate"    => Some(Transform::Rotate { angle: *args.first()?, cx: args.get(1).copied(), cy: args.get(2).copied() }),
                "skewx"     => Some(Transform::SkewX(*args.first()?)),
                "skewy"     => Some(Transform::SkewY(*args.first()?)),
                "matrix"    => if args.len() >= 6 { Some(Transform::Matrix(Matrix2D { a: args[0], b: args[1], c: args[2], d: args[3], e: args[4], f: args[5] })) } else { None },
                _ => None,
            }
        }).collect();
        match parts.len() { 0 => Transform::None, 1 => parts.remove(0), _ => Transform::Multiple(parts) }
    }
}
...
mod tests {
    use super::*;
    #[test] fn identity_concat() { let t = Matrix2D::translate(10.0, 20.0); let r = Matrix2D::identity().concat(t); assert!((r.e - 10.0).abs() < 1e-9); }
    #[test] fn rotate_90()       { let p = Matrix2D::rotate_deg(90.0).transform_point(Point::new(1.0, 0.0)); assert!((p.x).abs() < 1e-9); assert!((p.y - 1.0).abs() < 1e-9); }
